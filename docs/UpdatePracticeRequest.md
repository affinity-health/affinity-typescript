# UpdatePracticeRequest

## Properties

| Name                | Type                                                                                           |
| ------------------- | ---------------------------------------------------------------------------------------------- |
| `address`           | [UpdatePatientAddressResponseAddress](UpdatePatientAddressResponseAddress.md)                  |
| `attestations`      | [UpdatePracticeRequestAttestations](UpdatePracticeRequestAttestations.md)                      |
| `complianceContact` | [CreatePracticeRequestPrimaryContact](CreatePracticeRequestPrimaryContact.md)                  |
| `externalId`        | string                                                                                         |
| `legalName`         | string                                                                                         |
| `metadata`          | object                                                                                         |
| `name`              | string                                                                                         |
| `prescribers`       | [Array&lt;UpdatePracticeRequestPrescribersInner&gt;](UpdatePracticeRequestPrescribersInner.md) |
| `primaryContact`    | [CreatePracticeRequestPrimaryContact](CreatePracticeRequestPrimaryContact.md)                  |
| `supportEmail`      | string                                                                                         |
| `supportPhone`      | string                                                                                         |
| `timezone`          | string                                                                                         |

[[Back to top]](#) [[Back to API list]](../README.md#api-endpoints) [[Back to Model list]](../README.md#models) [[Back to README]](../README.md)
