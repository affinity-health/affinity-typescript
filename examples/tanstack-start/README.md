...
This independently packaged example installs the published `@affinity-health/sdk@1.10.0`.
It sends real requests to `https://api.joinaffinityai.com`, but accepts **Test keys only**.
...
| Name                           | Value                                                                                       |
| ------------------------------ | ------------------------------------------------------------------------------------------- |
| `AFFINITY_EXAMPLE_API_KEY`     | Dedicated Test-mode platform or practice API key for the deployed API                       |
| `AFFINITY_EXAMPLE_PRACTICE_ID` | A practice accessible to that key, using its `prac_` API ID, not an `org_` dashboard URL ID |
...
doppler secrets set --project affinity --config stg AFFINITY_EXAMPLE_PRACTICE_ID
...
1. Create a synthetic patient. Repeated clicks reuse the saved run's idempotency key.
